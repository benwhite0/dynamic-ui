...
export default async function ReportsPage() {
  ...
  return (
    // The layout is a fixed-height flex column, so this pane owns its scroll.
    <main className="w-full flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-6xl px-4 py-6">
        <div className="mb-6">
          <h1 className="text-lg font-medium text-foreground">AI spend</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {period} · {formatCurrency(currentTotal)} across {models.length}{" "}
            models and {teams.length} teams
          </p>
        </div>
